<script>
	(function($) {
		...
		$(function() {
			$("#w2gm-field-input-<?php echo $content_field->id; ?>-start").datepicker({
				changeMonth: true,
				changeYear: true,
				...
				onSelect: function(dateText) {
					var tmstmp_str;
					var sDate = $("#w2gm-field-input-<?php echo $content_field->id; ?>-start").datepicker("getDate");
					if (sDate) {
						sDate.setMinutes(sDate.getMinutes() - sDate.getTimezoneOffset());
						tmstmp_str = $.datepicker.formatDate('@', sDate)/1000;
					} else 
						tmstmp_str = 0;
					$("#w2gm-field-input-<?php echo $content_field->id; ?>-end").datepicker('option', 'minDate', sDate);
	
					$("input[name=w2gm-field-input-<?php echo $content_field->id; ?>-start]").val(tmstmp_str);
				}
			});
			<?php
			if ($lang_code = w2gm_getDatePickerLangCode(get_locale())): ?>
			$("#w2gm-field-input-<?php echo $content_field->id; ?>-start").datepicker($.datepicker.regional[ "<?php echo $lang_code; ?>" ]);
			<?php endif; ?>
			
			$("#w2gm-field-input-<?php echo $content_field->id; ?>-end").datepicker({
				changeMonth: true,
				changeYear: true,
				<?php if (function_exists('is_rtl') && is_rtl()): ?>isRTL: true,<?php endif; ?>
				showButtonPanel: true,
				...
				onSelect: function(dateText) {
					var tmstmp_str;
					var sDate = $("#w2gm-field-input-<?php echo $content_field->id; ?>-end").datepicker("getDate");
					if (sDate) {
						sDate.setMinutes(sDate.getMinutes() - sDate.getTimezoneOffset());
						tmstmp_str = $.datepicker.formatDate('@', sDate)/1000;
					} else 
						tmstmp_str = 0;
					$("#w2gm-field-input-<?php echo $content_field->id; ?>-start").datepicker('option', 'maxDate', sDate);
	
					$("input[name=w2gm-field-input-<?php echo $content_field->id; ?>-end]").val(tmstmp_str);
				}
			});
			<?php
			if ($lang_code = w2gm_getDatePickerLangCode(get_locale())): ?>
			$("#w2gm-field-input-<?php echo $content_field->id; ?>-end").datepicker($.datepicker.regional[ "<?php echo $lang_code; ?>" ]);
			<?php endif; ?>

			<?php if ($content_field->value['date_end']): ?>
			$("#w2gm-field-input-<?php echo $content_field->id; ?>-end").datepicker('setDate', $.datepicker.parseDate('dd/mm/yy', '<?php echo date('d/m/Y', $content_field->value['date_end']); ?>'));
			$("#w2gm-field-input-<?php echo $content_field->id; ?>-start").datepicker('option', 'maxDate', $("#w2gm-field-input-<?php echo $content_field->id; ?>-end").datepicker('getDate'));
			<?php endif; ?>
			$("#w2gm-reset-date-<?php echo $content_field->id; ?>-end").click(function() {
				$.datepicker._clearDate('#w2gm-field-input-<?php echo $content_field->id; ?>-end');
			})

			<?php if ($content_field->value['date_start']): ?>
			$("#w2gm-field-input-<?php echo $content_field->id; ?>-start").datepicker('setDate', $.datepicker.parseDate('dd/mm/yy', '<?php echo date('d/m/Y', $content_field->value['date_start']); ?>'));
			$("#w2gm-field-input-<?php echo $content_field->id; ?>-end").datepicker('option', 'minDate', $("#w2gm-field-input-<?php echo $content_field->id; ?>-start").datepicker('getDate'));
			<?php endif; ?>
			$("#w2gm-reset-date-<?php echo $content_field->id; ?>-start").click(function() {
				$.datepicker._clearDate('#w2gm-field-input-<?php echo $content_field->id; ?>-start');
			})
		});
	})(jQuery);
</script>
		...
		<div class="w2gm-row w2gm-form-group w2gm-clearfix">
			<div class="w2gm-col-md-3">
				...
				<div class="w2gm-has-feedback">
					<input type="text" id="w2gm-field-input-<?php echo $content_field->id; ?>-start" class="w2gm-field-input-datetime w2gm-form-control" />
					<span class="w2gm-glyphicon w2gm-glyphicon-calendar w2gm-form-control-feedback"></span>
					<input type="hidden" name="w2gm-field-input-<?php echo $content_field->id; ?>-start" value="<?php echo esc_attr($content_field->value['date_start']); ?>"/>
				</div>
				
				<?php _e('End', 'W2GM'); ?> 
				<div class="w2gm-has-feedback">
					<input type="text" id="w2gm-field-input-<?php echo $content_field->id; ?>-end" class="w2gm-field-input-datetime w2gm-form-control" />
					<span class="w2gm-glyphicon w2gm-glyphicon-calendar w2gm-form-control-feedback"></span>
					<input type="hidden" name="w2gm-field-input-<?php echo $content_field->id; ?>-end" value="<?php echo esc_attr($content_field->value['date_end']); ?>"/>
				</div>
			</div>
			<div class="w2gm-col-md-2">
				<br />
				<input type="button" class="w2gm-btn w2gm-btn-primary" id="w2gm-reset-date-<?php echo $content_field->id; ?>-start" value="<?php esc_attr_e('reset', 'W2GM')?>" />
				<br />
				<br />
				<input type="button" class="w2gm-btn w2gm-btn-primary" id="w2gm-reset-date-<?php echo $content_field->id; ?>-end" value="<?php esc_attr_e('reset', 'W2GM')?>" />
			</div>
		</div>
