...
class w2gm_comments_manager {
	
	public function __construct() {
		add_action('wp_ajax_w2gm_comments_add_comment', array($this, 'comments_add_comment'));
	    add_action('wp_ajax_nopriv_w2gm_comments_add_comment', array($this, 'comments_add_comment'));
	    add_action('wp_ajax_w2gm_comments_load_template', array($this, 'comments_load_template'));
	    add_action('wp_ajax_nopriv_w2gm_comments_load_template', array($this, 'comments_load_template'));
	}

	public function comments_load_template() {
		check_ajax_referer('w2gm_comments_nonce', 'security');
	...
	public function display_comments_node($post_id, $parent = 0) {
		...
		if ($comments):
		?>
		<div class="w2gm-comments-container">
	...
	public function display_comment($comment) {
		...
		?>
		<div class="w2gm-comments-content w2gm-comments-<?php echo $class; ?>" id="comment-<?php echo $comment->comment_ID; ?>">
			<div class="w2gm-comments-p">
				<?php w2gm_comments_profile_pic($comment->comment_author_email); ?>
				...
				<time class="meta">
					...
					<?php print human_time_diff(strtotime($comment->comment_date), current_time('timestamp')); ?> ago. <a class="w2gm-comment-reply" data-comment-id="<?php echo $comment->comment_ID; ?>" data-comment-author="<?php echo esc_attr($comment->comment_author); ?>" href="javascript: void(0);"><?php _e('Reply ↓', 'W2GM'); ?></a>
				</time>
			</div>
		</div>
		<?php 
...
function w2gm_comments_profile_pic($id_or_email = null, $email = null) {
	...
	echo '<span class="w2gm-comments-profile-pic-container">' . $html . '</span>';
}
