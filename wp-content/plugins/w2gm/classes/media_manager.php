...
class w2gm_media_manager {
	...
	public function __construct() {
		...
		add_action('wp_ajax_w2gm_remove_image', array($this, 'remove_image'));
		add_action('wp_ajax_nopriv_w2gm_remove_image', array($this, 'remove_image'));
		...
		if (get_option('w2gm_prevent_users_see_other_media')) {
	...
	public function mediaMetabox($post = null, $args = array()) {
		w2gm_renderTemplate('media_metabox.tpl.php' , array_merge(array(
				'images' => $this->images,
				'videos' => $this->videos,
				'logo_image' => $this->logo_image,
				'classes' => (!empty($args['args']['target']) ? 'w2gm-media-metabox-'.$args['args']['target'] : '')
			), $this->params
		));
	}
	...
	public function upload_image() {
		...
		if (wp_verify_nonce($_GET['_wpnonce'], 'upload_images') && isset($_FILES['browse_file']) && ($_FILES['browse_file']['size'] > 0) && isset($_GET['post_id']) && ($post_id = $_GET['post_id'])) {
			if ($post = get_post($post_id)) {
				if (current_user_can('edit_post', $post_id) || !$post->post->post_author) {
					$do_upload = apply_filters('w2gm_count_attachments', true, $post_id);
					if ($do_upload) {
						...
						$result['error_msg'] = __('Number of images exceed limit.', 'W2GM');
				} else
	...
	public function validateAttachments(&$errors) {
		if ($this->params['images_number'] || $this->params['videos_number']) {
			...
			if ($this->params['images_number']) {
				$validation->set_rules('attached_image_id[]', __('Attached images ID', 'W2GM'));
				$validation->set_rules('attached_image_title[]', __('Attached images caption', 'W2GM'));
				$validation->set_rules('attached_images_order', __('Attached images order', 'W2GM'));
				if ($this->params['logo_enabled']) {
					$validation->set_rules('attached_image_as_logo', __('Logo image selection', 'W2GM'));
				}
			}
			if ($this->params['videos_number']) {
				$validation->set_rules('attached_video_id[]', __('Attached video ID', 'W2GM'));
			}
	...
	public function avoid_post_type_switcher($pages) {
		if (w2gm_getValue($_POST, 'post_type') == W2GM_POST_TYPE) {
			return;
		}
		return $pages;
	}
	...
	public function admin_enqueue_scripts_styles() {
		if (get_option('w2gm_images_lightbox') || is_admin()) {
		...
		if (is_admin() && current_user_can('upload_files')) {
			...
			wp_register_script('w2gm_fileupload', W2GM_RESOURCES_URL . 'js/jquery.fileupload.js', array('jquery'), false, true);
			wp_register_script('w2gm_fileupload_iframe', W2GM_RESOURCES_URL . 'js/jquery.iframe-transport.js', array('jquery'), false, true);
			wp_enqueue_script('w2gm_fileupload');
			wp_enqueue_script('w2gm_fileupload_iframe');
