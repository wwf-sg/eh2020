...
class w2gm_content_field_fileupload extends w2gm_content_field {
	...
	public function configure() {
		...
		if (w2gm_getValue($_POST, 'submit') && wp_verify_nonce($_POST['w2gm_configure_content_fields_nonce'], W2GM_PATH)) {
			...
			$validation->set_rules('use_text', __('Default file title text', 'W2GM'), 'is_checked');
			$validation->set_rules('default_text', __('Default file title text', 'W2GM'));
			$validation->set_rules('use_default_text', __('Use default file title text', 'W2GM'), 'is_checked');
			$validation->set_rules('allowed_mime_types', __('Use default file title text', 'W2GM'));
			if ($validation->run()) {
				$result = $validation->result_array();
				if ($wpdb->update($wpdb->w2gm_content_fields, array('options' => serialize(array('use_text' => $result['use_text'], 'default_text' => $result['default_text'], 'use_default_text' => $result['use_default_text'], 'allowed_mime_types' => $result['allowed_mime_types']))), array('id' => $this->id), null, array('%d')))
				...
				w2gm_renderTemplate('content_fields/fields/fileupload_configuration.tpl.php', array('content_field' => $this));
			}
		} else
			w2gm_renderTemplate('content_fields/fields/fileupload_configuration.tpl.php', array('content_field' => $this));
	}
	...
	public function renderInput() {
		...
		if (!($template = w2gm_isTemplate('content_fields/fields/fileupload_input_'.$this->id.'.tpl.php'))) {
		...
		w2gm_renderTemplate($template, array('content_field' => $this, 'file' => $file));
	}
	
	public function validateValues(&$errors, $data) {
		global $w2gm_instance;
		$listing = $w2gm_instance->current_listing;

		$field_index_input = 'w2gm-field-input-' . $this->id;
		$field_index_text = 'w2gm-field-input-text-' . $this->id;
		$field_index_uploaded = 'w2gm-uploaded-file-' . $this->id;
		$field_index_reset = 'w2gm-reset-file-' . $this->id;
		...
		if ($this->canBeRequired() && $this->is_required) {
			if (empty($_POST[$field_index_uploaded]) && (empty($_FILES) || !isset($_FILES[$field_index_input]) || !$_FILES[$field_index_input]['tmp_name'])) {
				$errors[] = sprintf(__('%s is required to be uploaded!', 'W2GM'), $this->name);
				return false;
			}
		...
		if (!$upload_errors) {
			...
			if (!is_wp_error($upload_id)) {
				...
				$upload_errors[] = __('Error during file upload!', 'W2GM');
	...
	public function renderOutput($listing, $group = null) {
		...
		if (!($template = w2gm_isTemplate('content_fields/fields/fileupload_output_'.$this->id.'.tpl.php'))) {
		...
		w2gm_renderTemplate($template, array('content_field' => $this, 'listing' => $listing, 'file' => $file, 'group' => $group));
	}
	...
	public function validateCsvValues($value, &$errors) {
		...
		$id = $value[0];
		if (is_integer($id))
			$errors[] = __("ID of media attachment is invalid", "W2GM");
	...
	public function renderOutputForMap($location, $listing) {
		...
		return w2gm_renderTemplate('content_fields/fields/fileupload_output_map.tpl.php', array('content_field' => $this, 'listing' => $listing, 'file' => $file), true);
	}
