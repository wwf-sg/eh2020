...
include_once W2GM_PATH . 'classes/content_fields/fields/content_field_fileupload.php';
include_once W2GM_PATH . 'classes/content_fields/fields/content_field_phone.php';
...
class w2gm_content_fields {
	...
	public function __construct() {
		$this->fields_types_names = array(
				...
				'fileupload' => __('File upload', 'W2GM'),
		);
	...
	public function saveOrder() {
		...
		if (isset($_POST['content_fields_order']) && $_POST['content_fields_order'] && ($order_ids = explode(',', trim($_POST['content_fields_order'])))) {
			...
			foreach ($order_ids AS $id) {
				$wpdb->update($wpdb->w2gm_content_fields, array('order_num' => $i), array('id' => $id));
				$i++;
			}
			$this->getContentFieldsFromDB();
	...
	public function saveListingTabsOption() {
		...
		foreach ($vpt_option['w2gm_listings_tabs_order'] AS $key=>$option) {
			if (strpos($option, 'field-group-tab-') !== FALSE) {
				...
				if (!$fields_group->on_tab) {
					unset($vpt_option['w2gm_listings_tabs_order'][$key]);
				}
			}
		}
		...
		foreach ($this->content_fields_groups_array AS $fields_group) {
			if ($fields_group->on_tab) {
				if (!in_array('field-group-tab-'.$fields_group->id, $vpt_option['w2gm_listings_tabs_order'])) {
					$vpt_option['w2gm_listings_tabs_order'][] = 'field-group-tab-'.$fields_group->id;
				}
			}
		}
		update_option('vpt_option', $vpt_option);
		update_option('w2gm_listings_tabs_order', $vpt_option['w2gm_listings_tabs_order']);
	}
	...
	public function getNotCoreContentFields() {
		...
		$content_fields = apply_filters('w2gm_content_fields_metabox', $content_fields, $listing);
	...
	public function getFieldsByCategoriesIds($categories_ids) {
		/* if ($level_id) {
			global $w2gm_instance;
			$level = $w2gm_instance->levels->getLevelById($level_id);
		} else 
			$level = null;
	...
	public function getOrderParams($defaults = array()) {
		$order_by = w2gm_getValue($_GET, 'order_by', w2gm_getValue($defaults, 'order_by'));
	...
	public function renderInputByGroups($post) {
		...
		foreach ($content_fields_groups AS $group) {
			...
			if ($fields_to_render) {
				w2gm_renderTemplate('content_fields/fields_group_input.tpl.php', array('group' => $group, 'content_fields' => $fields_to_render, 'post' => $post));
			}
		}
		...
		if ($fields_to_render) {
			echo '<div class="w2gm-submit-section w2gm-submit-section-content-fields">
					<div class="w2gm-submit-section-inside">';
			w2gm_renderTemplate('content_fields/content_fields_metabox.tpl.php', array('content_fields' => $fields_to_render, 'post' => $post));
			echo '</div>
					</div>';
		}
	}
...
class w2gm_content_fields_group {
	...
	public function create($array) {
		...
		$insert_update_args = apply_filters('w2gm_content_field_group_create_edit_args', $insert_update_args, $this, $array);
...
class w2gm_content_field {
	...
	public function validation() {
		...
		if ($process_content_field->canBeOrdered())
			$validation->set_rules('is_ordered', __('Order by field', 'W2GM'), 'is_checked');
		$validation->set_rules('is_hide_name', __('Hide name', 'W2GM'), 'is_checked');
		$validation->set_rules('for_admin_only', __('For admin only', 'W2GM'), 'is_checked');
		$validation->set_rules('on_listing_sidebar', __('On listings sidebar', 'W2GM'), 'is_checked');
		$validation->set_rules('on_listing_page', __('On listing page', 'W2GM'), 'is_checked');
		...
		if ($process_content_field->isCategories())
			$validation->set_rules('categories', __('Assigned categories', 'W2GM'));
	...
	public function create($array) {
		...
		$insert_update_args = array(
				...
				'for_admin_only' => w2gm_getValue($array, 'for_admin_only'),
		...
		if ($this->canBeOrdered())
			$insert_update_args['is_ordered'] = w2gm_getValue($array, 'is_ordered');
		if ($this->isCategories())
			$insert_update_args['categories'] = serialize(w2gm_getValue($array, 'categories', array()));
	...
	public function buildContentFieldFromArray($array) {
		...
		$this->is_ordered = w2gm_getValue($array, 'is_ordered');
		$this->is_hide_name = w2gm_getValue($array, 'is_hide_name');
		$this->for_admin_only = w2gm_getValue($array, 'for_admin_only');
...
function w2gm_content_fields_names_into_strings() {
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		foreach ($w2gm_instance->content_fields->content_fields_array AS &$content_field) {
			$content_field->name = apply_filters('wpml_translate_single_string', $content_field->name, 'Google Maps locator', 'The name of content field #' . $content_field->id);
			$content_field->description = apply_filters('wpml_translate_single_string', $content_field->description, 'Google Maps locator', 'The description of content field #' . $content_field->id);
		}
		foreach ($w2gm_instance->content_fields->content_fields_groups_array AS &$content_fields_group) {
			$content_fields_group->name = apply_filters('wpml_translate_single_string', $content_fields_group->name, 'Google Maps locator', 'The name of content fields group #' . $content_fields_group->id);
		}
	}
}

add_filter('w2gm_content_field_create_edit_args', 'w2gm_filter_content_field_fields', 10, 3);
function w2gm_filter_content_field_fields($insert_update_args, $content_field, $array) {
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE) {
			if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['field_id'])) {
				...
				if ($name_string_id = icl_st_is_registered_string('Google Maps locator', 'The name of content field #' . $content_field_id))
					icl_add_string_translation($name_string_id, ICL_LANGUAGE_CODE, $insert_update_args['name'], ICL_TM_COMPLETE);
				if ($description_string_id = icl_st_is_registered_string('Google Maps locator', 'The description of content field #' . $content_field_id))
...
add_action('w2gm_update_content_field', 'w2gm_save_content_field', 10, 3);
function w2gm_save_content_field($content_field_id, $content_field, $array) {
	global $sitepress;

	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE && $content_field->isCategories()) {
			update_option('w2gm_wpml_content_field_categories_'.$content_field_id.'_'.ICL_LANGUAGE_CODE, w2gm_getValue($array, 'categories'));
		}
		
		if ($sitepress->get_default_language() == ICL_LANGUAGE_CODE) {
			do_action('wpml_register_single_string', 'Google Maps locator', 'The name of content field #' . $content_field_id, w2gm_getValue($array, 'name'));
			do_action('wpml_register_single_string', 'Google Maps locator', 'The description of content field #' . $content_field_id, w2gm_getValue($array, 'description'));
		}
	}
}

add_filter('w2gm_content_field_group_create_edit_args', 'w2gm_filter_content_field_group_fields', 10, 3);
function w2gm_filter_content_field_group_fields($insert_update_args, $content_field_group, $array) {
	global $sitepress, $wpdb;
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE) {
			if (isset($_GET['action']) && $_GET['action'] == 'edit_group' && isset($_GET['group_id'])) {
				...
				if ($name_string_id = icl_st_is_registered_string('Google Maps locator', 'The name of content fields group #' . $content_field_group_id))
					icl_add_string_translation($name_string_id, ICL_LANGUAGE_CODE, $insert_update_args['name'], ICL_TM_COMPLETE);
				unset($insert_update_args['name']);
			}
		}
...
function w2gm_save_content_field_group($content_field_group_id, $content_field_group, $array) {
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() == ICL_LANGUAGE_CODE) {
			do_action('wpml_register_single_string', 'Google Maps locator', 'The name of content fields group #' . $content_field_group_id, w2gm_getValue($array, 'name'));
		}
	}
}
