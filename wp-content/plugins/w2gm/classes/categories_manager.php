...
class w2gm_categories_manager {
	...
	public function __construct() {
		...
		add_action(W2GM_CATEGORIES_TAX . '_edit_form_fields', array($this, 'select_icon_form'));
		...
		if (w2gm_isCategoriesEditPageInAdmin()) {
			add_action('admin_enqueue_scripts', array($this, 'admin_enqueue_category_edit_scripts'));
		}

		add_action('wp_ajax_w2gm_select_category_icon_dialog', array($this, 'select_category_icon_dialog'));
		add_action('wp_ajax_w2gm_select_category_icon', array($this, 'select_category_icon'));
	...
	public function addCategoriesMetabox($post_type) {
		if ($post_type == W2GM_POST_TYPE && ($level = w2gm_getCurrentListingInAdmin()->level) && ($level->categories_number > 0 || $level->unlimited_categories)) {
	...
	public function validateCategories($level, &$postarr, &$errors) {
		...
		if (isset($postarr['tax_input'][W2GM_CATEGORIES_TAX]) && is_array($postarr['tax_input'][W2GM_CATEGORIES_TAX])) {
			if (!$level->unlimited_categories)
				...
				$postarr['tax_input'][W2GM_CATEGORIES_TAX] = array_slice($postarr['tax_input'][W2GM_CATEGORIES_TAX], 0, $level->categories_number, true);

			if ($level->categories && array_diff($postarr['tax_input'][W2GM_CATEGORIES_TAX], $level->categories))
				$errors[] = __('Sorry, you can not choose some categories for this level!', 'W2GM');
	...
	public function taxonomy_columns($original_columns) {
		...
		$new_columns['w2gm_category_icon'] = __('Icon', 'W2GM');
	...
	public function taxonomy_rows($row, $column_name, $term_id) {
		...
		if ($column_name == 'w2gm_category_icon') {
	...
	public function select_icon_form($term) {
		w2gm_renderTemplate('categories/select_icon_form.tpl.php', array('term' => $term));
	}
	public function choose_icon_link($term_id) {
		$icon_file = $this->getCategoryIconFile($term_id);
		w2gm_renderTemplate('categories/select_icon_link.tpl.php', array('term_id' => $term_id, 'icon_file' => $icon_file));
	}
	public function getCategoryIconFile($term_id) {
		return w2gm_getCategoryIconFile($term_id);
	}
	public function select_category_icon_dialog() {
		$categories_icons = array();
		
		$categories_icons_files = scandir(W2GM_CATEGORIES_ICONS_PATH);
		foreach ($categories_icons_files AS $file) {
			if (is_file(W2GM_CATEGORIES_ICONS_PATH . $file) && $file != '.' && $file != '..') {
				$categories_icons[] = $file;
			}
		}
		
		w2gm_renderTemplate('categories/select_icons_dialog.tpl.php', array('categories_icons' => $categories_icons));
		die();
	}
	public function select_category_icon() {
		if (isset($_POST['category_id']) && is_numeric($_POST['category_id'])) {
			$category_id = intval($_POST['category_id']);
			$icons = (array) get_option('w2gm_categories_icons');
			if (isset($_POST['icon_file']) && $_POST['icon_file']) {
				$icon_file = $_POST['icon_file'];
				if (is_file(W2GM_CATEGORIES_ICONS_PATH . $icon_file)) {
					$icons[$category_id] = $icon_file;
					update_option('w2gm_categories_icons', $icons);
					echo $category_id;
				}
			} else {
	...
	public function select_category_marker_png_image() {
		if (isset($_POST['category_id']) && is_numeric($_POST['category_id'])) {
			$category_id = intval($_POST['category_id']);
			$markers_images = (array) get_option('w2gm_categories_marker_images');
	...
	public function select_category_marker_icon() {
		if (isset($_POST['category_id']) && is_numeric($_POST['category_id'])) {
			$category_id = intval($_POST['category_id']);
			$markers_icons = (array) get_option('w2gm_categories_marker_icons');
	...
	public function select_category_marker_color() {
		if (isset($_POST['category_id']) && is_numeric($_POST['category_id'])) {
			$category_id = intval($_POST['category_id']);
			$markers_colors = (array) get_option('w2gm_categories_marker_colors');
	...
	public function admin_enqueue_category_edit_scripts() {
		...
		wp_localize_script(
				...
				'categories_icons',
				array(
						'categories_icons_url' => W2GM_CATEGORIES_ICONS_URL,
		...
		if ($listing = w2gm_getCurrentListingInAdmin()) {
			...
			wp_localize_script(
					...
					array(
							...
							'level_categories_notice_disallowed' => __('Sorry, you can not choose this category for this level!', 'W2GM'),
							'level_categories_notice_number' => sprintf(__('Please choose %d category!', 'W2GM'), $categories_number)
					)
			);
		}
	}
