 ...
 * @return void
 */
function w2gm_breadcrumb_trail( $args = array() ) {

	$breadcrumb = apply_filters('breadcrumb_trail_object', null, $args);

	if ( ! is_object( $breadcrumb ) )
		$breadcrumb = new W2GM_Breadcrumb_Trail( $args );
...
class W2GM_Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			if ( true === $this->args['show_browse'] ) {
				...
				$breadcrumb .= sprintf(
					'<%1$s class="w2gm-trail-browse">%2$s</%1$s>',
			...
			$breadcrumb .= sprintf(
				'<%s class="w2gm-trail-items" itemscope itemtype="http://schema.org/BreadcrumbList">',
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="w2gm-breadcrumb-trail w2gm-breadcrumbs" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'w2gm-breadcrumb-trail' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'w2gm-breadcrumb-trail' ),
			'home'                => esc_html__( 'Home',                                  'w2gm-breadcrumb-trail' ),
			'error_404'           => esc_html__( '404 Not Found',                         'w2gm-breadcrumb-trail' ),
			'archives'            => esc_html__( 'Archives',                              'w2gm-breadcrumb-trail' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'w2gm-breadcrumb-trail' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'w2gm-breadcrumb-trail' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'w2gm-breadcrumb-trail' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'w2gm-breadcrumb-trail' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'w2gm-breadcrumb-trail' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'w2gm-breadcrumb-trail' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'w2gm-breadcrumb-trail' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'w2gm-breadcrumb-trail' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'w2gm-breadcrumb-trail' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'w2gm-breadcrumb-trail' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'w2gm-breadcrumb-trail' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'w2gm-breadcrumb-trail' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'w2gm-breadcrumb-trail' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'w2gm-breadcrumb-trail' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'w2gm-breadcrumb-trail' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'w2gm-breadcrumb-trail' ) ) ) );
